...
## Fabrica de puntas de functor.

Antes de ir mas lejos, tengo una confesion que hacer: No he sido completamente honesto acerca del metodo `of` que hemos puesto en cada uno de nuestros tipos. Resulta que, esto no esta ahi solamente para evitar la palabra clave `new`, sino mas bien para colocar los valores en lo que se llama *un contexto minimo por defecto*. Si, `of` en realidad no toma el lugar de un constructor- es parte de una interfaz importante que llamamos *Pointed* 

  Un *functor pointed* es un functor con un metodo `of`

Lo que es importante aqui es la capacidad de soltar cualquier valor en nuestro tipo y comenzar el mapeo en sentido opuesto.
